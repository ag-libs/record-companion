 ...
 * 
 * Processes @Builder annotations on record classes and generates
 * companion classes with builder pattern implementations.
 */
...
public class RecordCompanionProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(Builder.class)) {
            ...
            try {
                builderGenerator.generateCompanionClass(recordElement);
            } catch (IOException e) {
                processingEnv.getMessager().printMessage(
                    Diagnostic.Kind.ERROR,
                    "Failed to generate companion class for " + recordElement.getSimpleName() + ": " + e.getMessage(),
                    recordElement
                );
            }
        }
